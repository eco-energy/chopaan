...
module Data.BTreeIndex where

-- $ Cribbed off of Ben Gamari.
...
import Data.BTreeIndex.Types

-- | A Producer which accepts offsets for the yielded objects in return
-- type DiskProducer m a = UF.Unfold m (OnDisk a) a

-- -- putBS :: (Serialise a, Monad m) => a -> UF.Unfold m (OnDisk a) (A.Array Word8)
-- -- putBS a0 = {-# SCC "putBS" #-} evalStateT (go a0) 0
-- --   where
-- --     go a = do
-- --         s <- get
-- --         let bs = SBS.toArray $ W.serialise a
-- --         put $! s + fromIntegral (A.byteLength bs)
-- --         lift $ yield bs
...
-- buildTree :: (Monad m, Serialise e, Serialise k)
--           => Order -> Size
...
-- fromOrderedToFile :: (MonadMask m, MonadIO m, Serialise e, Serialise k)
--                   => Order                     -- ^ Order of tree
...
-- fromOrderedToByteString :: (Monad m, Serialise e, Serialise k)
--                         => Order                     -- ^ Order of tree
--                         -> Size                      -- ^ Maximum tree size
--                         -> Producer (BLeaf k e) m r  -- ^ 'Producer' of elements
--                         -> m LBS.ByteString
