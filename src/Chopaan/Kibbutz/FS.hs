...
newtype IsoM m a b = IsoM { unIsoM :: ((a -> m b), (b -> m a)) }

isoFwd = fst . unIsoM
isoRev = snd . unIsoM

type FPArrIso m = IsoM m FilePath ArrPath

type PathM m = (MonadThrow m, MonadIO m) 

fpArrIso :: (PathM m) => FPArrIso m
fpArrIso = IsoM (arrPath, fromArrPath)
  where
    arrPath = fmap ArrPath . (liftIO . Array.fromStreamD
              . S.toStreamD . encodeUtf8 @IO @S.SerialT . S.fromList)
    fromArrPath = (fmap toFilePath . parseRelFile <=< S.toList) . decodeUtf8
...
arrDirPath = IsoM (arrPath, fromArrPath)
  where
    arrPath = fmap ArrPath . liftIO . Array.fromStreamD
              . S.toStreamD . encodeUtf8 @IO @S.SerialT . S.fromList . toFilePath
    fromArrPath = (parseRelDir <=< S.toList) . decodeUtf8
                  . Array.toStream . unArrPath
...
getEv :: PathM m => Event -> m (Maybe (Either KbtzEv NodeEv))
getEv ev = case (EvL.isDir ev) of
  True -> (fmap Left) <$> getKbtzEv ev
  False -> (fmap Right) <$> getNodeEv ev
...
evRelPath :: (PathM m) => EvL.Event -> m FilePath
evRelPath = isoRev fpArrIso . ArrPath . EvL.getRelPath
...
getKbtzEv ev
  | EvL.isDir ev && (EvL.isCreated ev || EvL.isDeleted ev) = do
      --liftIO $ ((\p -> print $ "kbtzEv: " <> p) =<< evRelPath ev)
      k <- toKbtzName . EvL.getRelPath $ ev
      case k of
        Nothing -> return Nothing
        Just k' -> return $ ctor k' 
...
dirnameC :: Path t Dir -> T.Text
dirnameC = T.init . T.pack . toFilePath . dirname

toKbtzName' :: Path t Dir -> KbtzName
toKbtzName' = KbtzId . dirnameC

toKbtzName'' :: Maybe (Either RelDir RelFile) -> Maybe KbtzName
toKbtzName'' = ((either (Just . toKbtzName') (const Nothing)) =<<)

toKbtzName :: (PathM m) => Array.Array Word8 -> m (Maybe KbtzName)
toKbtzName = fmap (Just . toKbtzName') . isoRev arrDirPath . ArrPath
...
getNodeEv ev = do
  ...
  where
    ctor
      ...
      | EvL.isAccessed ev = Just . uncurry ReadNode
      | EvL.isModified ev = Just . uncurry UpdateNode
      | EvL.isDeleted ev = Just . uncurry DeleteNode
      | otherwise = const Nothing
