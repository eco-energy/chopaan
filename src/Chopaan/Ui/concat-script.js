...
//       console.log("pan = (" + panX + "," + panY + ")" );
  ...
  var onresize = function (event) {
      ...
      //console.log("canvas width: ", w, "\ncanvas height:", h, "pixel size:", pixel_size);
  ...
  canvas.onmousemove = debounce(on_move, 250, true);
...
function mkSlider(props,onChange) {
   ...
   function to_rval (ival) { return ival / 100 * width + props.min; };
   // map [min,max] to [0,10000]
   function to_ival (rval) { return (rval - props.min) / width * 100; };
   ...
   slider.slider({
      value: to_ival(props.value), min: 0, max: 100,
