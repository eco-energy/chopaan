{-# LANGUAGE ScopedTypeVariables, TypeApplications, BangPatterns, RecordWildCards, ExistentialQuantification, RankNTypes, QuantifiedConstraints, CPP, StrictData, FunctionalDependencies, FlexibleContexts #-}
...
energyFold = fst <$> FL.foldl' step begin
  where
    -- forall s. Fold (s -> a -> m s) (m s) (s -> m b)
    {-# INLINE step #-}
...
batteryFold cBat bat@BatteryParams{} = bimap toWattSeconds toWatts . flip end emptyB
...
sensorFold hw = FL.lmap clampES . FL.toFold $ SensorMetrics
                <$> FL.Tee (fst <$> timeFold)
                <*> FL.Tee (snd <$> timeFold)
                <*> FL.Tee powerFold
                <*> FL.Tee energyFold
