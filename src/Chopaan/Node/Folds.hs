...
timeFold = FL.Fold step' begin' done'
  where
    step' :: Timestamp -> EnergyState -> m (FL.Step Timestamp Timestamp)
    step' (Nothing, _) cur = pure . FL.Partial $ (Just tn, diffUTC tn tn)
      where
        tn = utcTimeES cur
    step' ((Just !prev), _) cur = pure . FL.Partial $ (Just tn, diffUTC tn prev)
      where
        tn = utcTimeES cur
    begin' :: m Timestamp
...
energyFold = AppF (FL.Fold step begin end)
  where
    ...
    step (esPrev, (Just tPrev)) cur = pure . FL.Partial $
      ...
      where
        tn = utcTimeES cur
    step (esPrev, Nothing) cur = pure . FL.Partial $
...
batteryFold bat@BatteryParams{} = fmap (bimap toWattSeconds toWatts) (FL.Fold step begin end)
  where
    step :: (Maybe UTCTime, Maybe (KF R))
      -> EnergyState
      -> m (FL.Step (Maybe UTCTime, Maybe (KF R)) (Battery R R))
    step (t, kf) sensorReadings = FL.Partial
                                  <$> ((\(_, b) -> (Just tnow, Just b)) . snd)
                                  <$>
...
demandFold = FL.Fold (\_ nes -> pure . FL.Partial $ (d $ power nes)) (pure 0) pure
  where
