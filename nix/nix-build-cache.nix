...
drv.overrideAttrs (oldAttrs: {
  ...
  preConfigure = ''
    ...
    (
      ${pkgs.awscli}/bin/aws s3 cp "${s3-uri cache-name}" "$CACHE_TAR" --region=${aws-region}\
        || ${pkgs.awscli}/bin/aws s3 cp "${s3-uri master-cache}" "$CACHE_TAR" --region=${aws-region}
      tar xf "$CACHE_TAR"
      rm "$CACHE_TAR"
    ...
    function uploadCache() {
      ...
      (
        ...
        ${pkgs.awscli}/bin/aws s3 cp "$CACHE_TAR" "${s3-uri cache-name}" --region=${aws-region}
        rm "$CACHE_TAR"
