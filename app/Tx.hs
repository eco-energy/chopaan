...
main = do
  ...
  let txs = txTest 100 nodeId
  forkIO $ forever $ (pub cl outbox)
  mapM_ (\t -> dispatchTx outbox t >> (threadDelay $ 1000*1000) >> print ("Dispatched! ")) txs


txTest :: Int -> NodeMAC -> [Tx NodeMAC]
txTest n addr = [ testTx addr | _ <- enumFromTo 0 n ]



testTx :: NodeMAC -> Tx NodeMAC
testTx addr = Tx $ Map.fromList [(addr, mkStake Source 100 100)]
